...
export class AccountService {
  ...
  register(data: AccountDTO): Observable<any> {
  ...
  getRole(): Observable<{ role: string }> {
    return this.http.get<{ role: string }>(`${this.baseUrl}/role`, this.getAuthHeaders());
  }

  getCurrentUser(): Observable<any> {
    return this.http.get(`${this.baseUrl}/current`, this.getAuthHeaders());
  }
  ...
  checkPhone(phone: string): Observable<boolean> {
    return this.http.get<boolean>(`${this.baseUrl}/exists/phone`, {
      params: { phone }
    });
  }
