...
export class AddressesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  load() { this.loading = true; this.service.getMine().pipe(takeUntilDestroyed(this.destroyRef)).subscribe({ next: (x) => { this.addresses = x; this.loading = false; this.cdr.detectChanges(); }, error: () => { this.loading = false; this.cdr.detectChanges(); } }); }
  ...
  setDefault(a: AddressBookDTO) { if (!a.id) return; this.service.setDefault(a.id).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({ next: () => this.load() }); }
  delete(a: AddressBookDTO) { if (!a.id) return; this.service.delete(a.id).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({ next: () => this.load() }); }
